...
$(function () {
  ...
  if ($footerBtn.length === 0) return; // 要素がなければ処理しない
  ...
  $(window).on("scroll", function () {
    if ($(this).scrollTop() > 300) {
      $footerBtn.css({ opacity: 1, pointerEvents: "auto" });
    } else {
      $footerBtn.css({ opacity: 0, pointerEvents: "none" });
    }
  });

  // クリック時の動作（例）
  $footerBtn.on("click", function () {
    alert("PLAY NOW ボタンがクリックされました！");
  });
});
